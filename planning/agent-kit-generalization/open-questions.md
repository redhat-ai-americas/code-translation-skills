...
3. ~~**How much of `code-translation-skills/docs/references/python-migration/` gets copied forward?**~~ **Resolved (2026-04-09):** All Python-migration reference files moved to `legacy/docs/references/python-migration/`, including SUB-AGENT-GUIDE.md (which contained hardcoded py2to3 script references and Python-specific verification steps). The generic sub-agent delegation patterns from that guide are worth extracting into a new language-agnostic reference when the new kit needs one, but the old file should not be carried forward verbatim.
